...
def remove_close_rows(df, threshold=5000, min_points=4):
    ...
    # Capture the rows to be removed
    deleted_rows = df.loc[indices_to_remove]

    # Remove rows from the original df that fall within any window
...
# Reset the index after filtering - write the results to tsv files
...
# Creating two new columns to check changes of phase respective to the above and below markers
...
df_SNP.to_csv(snakemake.output.snps, sep='\t', header=True, index=False) # Writing the results to a tsv file
...
# A "ResMidPoint" column is created and the values are propagated to both bounds
